<template>
    <ion-page>
        ...
        <ion-content :fullscreen="true" class="ion-padding">
            ...
            <div id="container">
                <!-- 输入框 -->
                <ion-input placeholder="title" ref="titleRef" :autofocus="true" :counter="true"
                    :maxlength="64"></ion-input>
                <ion-textarea placeholder="remark" :auto-grow="true" :counter="true" :maxlength="64">
                </ion-textarea>

                <!-- 选择项 -->
                <groups-select :groups-value="groupsValue"></groups-select>
                <priority-select :priority-value="priorityValue"></priority-select>
                <repeat-select :repeat-value="repeatValue"></repeat-select>

                <!-- 日期 -->
                <ion-accordion-group><ion-accordion>
                        <ion-item slot="header" color="light">
                            <ion-label><ion-label>start date</ion-label>
                                {{ startDate }}
                            </ion-label>
                            <ion-label><ion-label>end date</ion-label>
                                {{ endDate }}
                            </ion-label>
                        </ion-item>

                        <div class="ion-padding" slot="content" id="date">
                            <ion-datetime presentation="date" :multiple="true" @ionChange="handleDateChange"
                                :highlighted-dates="highlightedDates" ref="datetime">
                            </ion-datetime>
                        </div>
                ...
                <!-- 按钮 -->
                <!-- 新建分组 -->
                <ion-button @click="openModal" color="light">add groups</ion-button>
...
async function handleDateChange() {
    ...
    const dates = datetime.value.$el.value.slice(-2); // 仅保留最后两个日期
    if (dates.length == 1) {
        startDate.value = endDate.value = dates[0];
        return;
    }
